...
import static com.example.umerejaz.testapp.R.id.btn_no;
import static com.example.umerejaz.testapp.R.id.next;

public class Ask1 extends AppCompatActivity implements View.OnClickListener{
        ...
        ParseQuery.getQuery(tableName).whereEqualTo("userID", ParseUser.getCurrentUser())
                .getFirstInBackground(new GetCallback<ParseObject>() {
                    ...
                    public void done(ParseObject obj1, final ParseException e) {
                        if (e == null) {
                            obj1.put("ownVehicle", num);
                            obj1.saveInBackground(new SaveCallback() {
                                @Override
                                public void done(ParseException e) {
                                    if (e == null) {
                                        Toast.makeText(Ask1.this, "Your Answer save successfully", Toast.LENGTH_SHORT).show();
                                        td.dismiss();
                                        Intent i = new Intent(Ask1.this, Ask2.class);
                                        i = i.putExtra("tittle", tittle);
                                        startActivity(i);
                                        if (tittle.equals("Trust|Estate Of Deceased Ind")) {
                                            i = new Intent(Ask1.this, Ask6.class);
                                            i = i.putExtra("tittle", tittle);
                                            startActivity(i);
